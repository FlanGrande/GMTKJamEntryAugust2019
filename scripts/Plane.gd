...
const PLANE_SPEED = 3 # speed of the plane
...
func _ready():
	timer.set_wait_time(rand_range(3, 7))
	timer.start()
...
func _on_timer_PlaneBomb_timeout():
	...
	if(can_drop_bombs):
		...
		bomb_instance.position = Vector2(position.x + 50, position.y + 25)
		get_parent().get_node("Background/Bombs").add_child(bomb_instance)
